    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for(int i = 0; i < jb.getItemDatas().size(); i ++) {
                udd.setItemCode(jb.getItemDatas().get(i).getProductCode());
                
                // insert_buy
                UserDataDAO.getInstance().buy(udd, jb.getItemDatas().get(i).getProductPrice());
            }
            jb.clearCart();
            
            KagoyumeHelper.getInstance().moveTop(jb);
            jb.setBuyCompleteDisplay(true);
